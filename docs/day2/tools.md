...
Оценив плюсы и минусы, поймём, что Конфлюенс приемлем для мануалов, но совершенно не годится для референсов.

Моё убеждение: все пользуются Конфлюенсом только потому, что нет альтернативы.
...
Если пишете референсы руками — Гитбук сойдёт. Для мануалов — очень даже ок.
...
**[Jekyll](https://jekyllrb.com/).** Самый популярный генератор статических сайтов. Позиционируется как решение в первую очередь для блогов. Поэтому в документации он интересен нам в первую очередь для мануалов.
...
-   [Sphinx domains](www.sphinx-doc.org/en/latest/usage/restructuredtext/domains.html)
...
**[Pandoc](https://pandoc.org/).** Конвертер из всего во всё. Строго говоря, это не генератор документации, хоть и может использоваться в таком качестве.
...
**[Foliant](https://foliant-docs.github.io/docs/).** Бессовестный пиар :-) Фолиант создавался как конкурент Сфинкса на Маркдауне. Имеет модульную архитектуру: всё интересное происходит в расширениях, а ядро выступает клеем.
...
Документация — это необязательно сайтик или документ. Компания Etsy разделила документацию на why docs и how docs. Первые меняются редко, они как раз на сайте. Вторые меняются постоянно, и их с сайта унесли в Слак. [Почитать статью](https://codeascraft.com/2018/10/10/etsys-experiment-with-immutable-documentation/).

В качестве эксперимента я сделал приложения для Слака и поставил его в своей команде. Скажу честно: пользуемся мало, но всё равно прикольно.

Попробуйте! Экспериментируйте!
