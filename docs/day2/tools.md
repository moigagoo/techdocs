...
Сейчас даже сам Гитхаб, кажется, забил на вики и больше пиарит GitHub Pages. Преимуществ у вики перед статическим сайтом нет.
...
-   популярный,
-   можно собирать документацию из комментариев в коде.
...
**[RAML](https://raml.org/).** То же самое, только спецификация другая.
...
**[API Blueprint](https://apiblueprint.org/).** Инструмент для документирования REST API в отрыве от кода вообще. Идеология — «спецификация первична, код — вторичен».

Как инструмент для описания абстрактных API-спецификаций он прекрасен: это обычный Маркдаун, который красиво рендерится.
...
**[Sphinx](http://www.sphinx-doc.org/).** В Сфинксе есть понятие [доменов](http://www.sphinx-doc.org/en/latest/usage/restructuredtext/domains.html). Домен — это набор reST-ролей для документирования кода на определённом языке программирования. Из коробки Python, JS, C, C++, reST.

Есть и [домен для REST API](https://sphinxcontrib-httpdomain.readthedocs.io/en/stable/).

Домены позволяют писать API-документацию вручную и генерировать из кода: на Питоне — из коробки, Go, .NET, JS — через [Sphinx AutoAPI](https://sphinx-autoapi.readthedocs.io/), Java — через [javasphinx](https://bronto.github.io/javasphinx/).

REST API можно генерировать из приложений на Flask, Bottle и Tornado.
...
[Компания Etsy разделила документацию на why docs и how docs](https://codeascraft.com/2018/10/10/etsys-experiment-with-immutable-documentation/). Первые меняются редко, их ведут как обычную документацию. Вторые меняются постоянно, и их с сайта перенесли в Slack. Одна команда добавляет статью в хранилище, другая — ищет по добавленным статьям.

В качестве эксперимента я сделал приложения для Слака и поставил его в своей команде. Чтобы поискать по базе знаний, пишем в чат: «/как зайти на прод такого-то сервиса?». Чтобы добавить ответ: «/так зайти на прод такого-то сервиса».

Пробуйте новые формы, экспериментируйте. Правил нет, деление на мануалы и референсы — условность из старых времён. Сейчас можно всё.
